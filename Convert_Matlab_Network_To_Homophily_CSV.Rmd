...
```{r}
### I THINK THIS IS A OLD RELIC, NOT NEEDED FOR CODE ###
for (i in 1:6) {
   graph_list[[i]] <- set_vertex_attr(graph = graph_list[[i]], name = 'GPA', value = node_list[paste0('GPA_', i)])
}
##########################################################
```
...
```{r}
write.csv(node_list,'Data_folder/homophily_data.csv')
```
